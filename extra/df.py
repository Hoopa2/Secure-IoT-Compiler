...
class DataFlowAnalyzer:
    ...
    def _mark_secret_sources(self):
        ...
        SAFE_VALUE_PREFIXES = ['mqtts://', 'coaps://', 'https://', 'mqtt://',
                               'coap://', 'http://']
        # Function calls that safely retrieve credentials (not hardcoded)
        SAFE_RETRIEVAL_FNS  = ['secure_storage_read', 'nvs_read_string',
                               'secure_get_', 'tls_get_', 'secure_provisioning_get']

        for line_num, line in enumerate(self.lines, 1):
            # Skip lines that retrieve from secure storage
            if any(fn in line for fn in SAFE_RETRIEVAL_FNS):
                continue
            ...
            if match:
                ...
                # Skip safe URL values
                if any(secret_value.startswith(p) for p in SAFE_VALUE_PREFIXES):
                    continue
            ...
            if match:
                var_name    = match.group(1)
                secret_value = match.group(2)
                # Skip safe URLs even if variable name looks secret-like
                if any(secret_value.startswith(p) for p in SAFE_VALUE_PREFIXES):
                    continue
