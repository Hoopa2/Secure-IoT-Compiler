...
class Lexer:
    ...
    def detect_weak_random(self):
        ...
        SECURE_RNG = ['esp_random', 'getrandom', 'RAND_bytes', 'arc4random',
                      'SecRandomCopyBytes', 'BCryptGenRandom']
        issues = []
        for line_num, line in enumerate(self.lines, 1):
            # Skip lines that already use a secure RNG
            if any(s in line for s in SECURE_RNG):
                continue
            for rng in WEAK_RANDOM_FUNCTIONS:
                # Word-boundary match so 'random()' inside 'esp_random()' is not matched
                if re.search(rf'\b{re.escape(rng)}\s*\(', line):
                    issues.append({
                        "type": "WEAK_RANDOM",
